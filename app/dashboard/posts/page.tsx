...
// import RouteProtector from "@/api/RouteProtector";  // Moved to layout
// import DarkModeToggle from "@/components/global/DarkModeToggle"; // commented out
    ...
    <>
      ...
      <ul className={twMerge("space-y-4 max-w-4xl")}>
        {localPosts.map((post) => (
          <li
            key={post.id}
            className={twMerge("p-4 rounded shadow bg-white", "text-gray-900")}
          >
            <h3 className="text-lg font-semibold mb-1">{post.title}</h3>
            <p className="mb-2 text-gray-700">{post.body}</p>
            <p className="text-sm text-gray-500 mb-3">User ID: {post.userId}</p>
            {user?.role === "admin" && (
